...
export default function CardTypePet() {
  ...
  const handleTypeClick = (title: string) => {
    ...
    // userRef.update({ typePets: newInterestedTypes ? true : null }, (error) => {
    //   if (error) {
  ...
  const handleSendClick = () => {
    const dataArray = [interestedTypes];
    const userId = https://stackoverflow.com/questions/67700374/use-localstorage-getitem-with-typescript
    // Call the function that updates the database with the dataArray
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', height: '100vh' }}>
      <Card sx={{ maxWidth: '90%', margin: '0 auto' }}>
        ...
        <CardActions sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}> 
          <Button variant="contained" onClick={handleSendClick} endIcon={<SendIcon />} >
            Send
          </Button>
        </CardActions>
      </Card>
