...
export default function SignInSide() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      setFormData({ email: data.get('email') as string, password: data.get('password') as string });
      try {
    ...
    const handleForgotPassword = async () => {
      if (formData.email) {
      await firebase.auth().sendPasswordResetEmail(formData.email.toString());
      } else {
                ...
                <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
                    ...
                    >
                        ...
                        <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 1 }}>
                            ...
                            <Grid container>
                                <Grid item xs>
                                <Link href="#" variant="body2" onClick={handleForgotPassword}{"Forgot password?"}
                                </Link>
                                </Grid>
                                <Grid item>
