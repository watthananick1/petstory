    ...
    if (newFiles) {
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        if (file.type.startsWith('image/') && file.size <= MAX_IMAGE_SIZE) {
          // add image file if it meets size requirements
          totalSize += file.size;
          if (validFiles.length < MAX_FILES && totalSize <= MAX_FILES * MAX_IMAGE_SIZE) {
            validFiles.push(file);
          }
        } else if (file.type.startsWith('video/') && file.size <= MAX_VIDEO_SIZE) {
          // add video file if it meets size requirements
