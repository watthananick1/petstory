...
import { collection, addDoc, updateDoc, getDoc, doc, DocumentReference, getFirestore, setDoc, FieldValue } from "firebase/firestore";
...
const Cards = ({ className }: CardsProps) => {
    ...
    const [tags, setTags] = useState<TypePet[]>([]);
    ...
    const handleTagSelect = (tagName: string) => {
      ...
      const handleSubmit = async (event: { preventDefault: () => void }) => {
        ...
        try {
          ...
          const userDataResponse = await axios.get(`http://localhost:4000/users/${userId}`);
          ...
          // Save the updated post data to Firestore
            try {
                const collectionRef = collection(db, "Post"); // replace "posts" with the actual name of your collection
                const newDocRef = doc(collectionRef); // this will create a new document with a unique ID
        ...
        >
            ...
            <div>
              {selectedTags.map((tags, i) => (
                <Chip key={initializeApp} label={tags.nameType} onClick={() => handleTagSelect(tags.nameType)} />
              ))}
            </div>
            ...
            >
                ...
                <div>
                  {tags.map(tags => (
                    <Chip key={tags.id} label={tags.nameType} onClick={() => handleTagSelect(tags.nameType)} />
                  ))}
                </div>
                <Button
                    variant="contained"
