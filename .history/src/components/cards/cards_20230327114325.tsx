...
import { collection, addDoc, updateDoc, getDoc, doc, DocumentReference, getFirestore } from "firebase/firestore";
...
https://chat.openai.com/chat/e65fb521-7916-487e-aa83-ad96ffdb4a18
...
const Cards = ({ className }: CardsProps) => {
    ...
    const handleSubmit = async (event: { preventDefault: () => void }) => {
        ...
        const postRefPath = `Post/${post}`; // Replace "Post" with the name of your collection and "post" with the ID of the post to be edited
          const postRef = doc(db, postRefPath);
          const postData = (await getDoc(postRef)).data() as PetPost;
          ...
          const promises = files.map(async (file) => {
            const storageRef = ref(storage, postRef.id + "/" + file.name);
      ...
      // Add the URLs of the uploaded files to the post data
      if (postData) {
        updatedPost.img = [...postData?.img, ...fileUrls];
      } else {
        updatedPost.img = fileUrls;
      }
