...
const Cards = ({ className }: CardsProps) => {
    ...
    const post = {
      ...
      const handleSubmit = async () => {
        // Create a new document in the "petPosts" collection
        const docRef = await addDoc(collection(db, 'petPosts'), {
            firstName: 'John',
            lastName: 'Doe',
            description: text,
            tagpet: chipData,
        ...
        // Upload the media files to the storage bucket
        const promises = files.map(async (file, index) => {
            const storageRef = ref(storage, `images/${docRef.id}/${index}`);
        ...
        // Update the document with the URLs of the uploaded files
        await updateDoc(doc(db, 'petPosts', docRef.id), {
            img: urls,
            postId: docRef.id,
        });
