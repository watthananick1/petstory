  ...
  const handleFileChange: ChangeEventHandler<HTMLInputElement> = (event) => {
    ...
    if (newFiles) {
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        if (file.type.startsWith('image/') && file.size <= ERROR in src/components/cards/cards.tsx:33:60
        TS2552: Cannot find name 'MAX_IMAGE_SIZE'. Did you mean 'MAX_IMAGE_FILES'?
            31 |       for (let i = 0; i < newFiles.length; i++) {
            32 |         const file = newFiles[i];
          > 33 |         if (file.type.startsWith('image/') && file.size <= MAX_IMAGE_SIZE) {
               |                                                            ^^^^^^^^^^^^^^
            34 |           validImageFiles.push(file);
            35 |         } else if (file.type.startsWith('video/') && file.size <= MAX_VIDEO_SIZE) {
            36 |           validVideoFile = file;
        
        ERROR in src/components/cards/cards.tsx:93:21
        TS2552: Cannot find name 'handleTextChange'. Did you mean 'handleFileChange'?
            91 |           placeholder="What's on your mind?"
            92 |           value={text}
          > 93 |           onChange={handleTextChange}
               |                     ^^^^^^^^^^^^^^^^
            94 |           fullWidth
            95 |           InputProps={{
